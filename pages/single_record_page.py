...
def build_card(card_lists):
    ...
    for item in card_lists[0]:
        ...
        single_row = dbc.Row([col_left, col_right], style={'padding': '3px', 'margin': '0px'})
        all_rows.append(single_row)
    title = html.I(card_lists[1])
    card = html.Div(
        children=[
            dbc.CardHeader(title, style={'text-align': 'center', 'padding': '0px'}),
            dbc.CardBody(all_rows, className='border-bottom')
        ],
    )
    return card
...
def build_geo(flag):
    ...
    card = html.Div(
        children=[
            ...
            dbc.CardBody(
                children=[
                    (html.Img(src=flag, width=40, style={'text-align': 'center',
                                                         'float': 'right'})),
...
class SingleRecord:
    ...
    def __init__(self):
        """A class that represents a 'Single Record' page template"""
        self.page = html.Div(
            children=[
                html.Br(),
                dbc.Row(
                    children=[
                        dbc.Col(
                            dbc.Card(
                                children=[
                                    build_card(user_card),
                                    build_card(query_card),
                                    build_card(geo_card),
                                ],
                                style={'padding': '0px'},
                            ),
